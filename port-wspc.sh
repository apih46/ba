...
tls="$(cat ~/log-install.txt | grep -w "VMess GRPC TLS" | cut -d: -f2|sed 's/ //g')"
echo -e "======================================"
echo -e ""
echo -e "     [1]  Change Port VMess GRPC TLS $tls"
echo -e "     [x]  Exit"
echo -e "======================================"
echo -e ""
...
case $prot in
1)
read -p "New Port VMess GRPC TLS: " tls1
if [ -z $tls1 ]; then
...
if [[ -z $cek ]]; then
sed -i "s/$tls/$tls1/g" /etc/v2ray/vmess.json
sed -i "s/   - V2RAY VMess GRPC TLS    : $tls/   - V2RAY VMess GRPC TLS    : $tls1/g" /root/log-install.txt
